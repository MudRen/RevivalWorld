...
void create()
{
	...
	// 技能英文
	set("id",		"waterclassify");

	// 技能頭銜
	set("title/40",		HIC"清水分類員"NOR);
	set("title/100",	HIC"清水分類師"NOR);
	
	// 技能註解
	set("note",		"清水工業分類技術(需清水採集Lv80)");
}
...
varargs int allowable_learn(object ob)
{
	...
	else
	{
		...
		{
			if( ob->query_skill_level("water") < 80 )
				return 0;
			else
				return 1;
		}
